...
episodes.forEach((episode) => {
  ...
  const seen = new Set();
  const boundedCuts = cuts
    .sort((left, right) => Number(left.at) - Number(right.at) || Number(right.score || 0) - Number(left.score || 0))
    .filter((cut) => {
      const key = `${Math.round(Number(cut.at))}|${String(cut.category || cut.label || "").toLowerCase()}`;
      if (seen.has(key)) return false;
      seen.add(key);
      return true;
    })
