...
function fixture() {
  ...
  const result = window.WWAMSourceDossierAdapter.build({
    ...
    channel: {
      ...
      packFingerprint: "fnv1a32:recovery-batch2-integration",
    },
  });
  return { window, result };
}

test("adds Batch 02 to Ask/Show Wiki evidence without changing the 510-source canon", () => {
  const { window, result } = fixture();
  const recoveryIds = new Set(
    window.WWAM_ARCHIVE_RECOVERY_BATCH2.streams.map((stream) => stream.id),
  );
  ...
  const recaps = result.sources.map((source) => source.episodeRecap);
