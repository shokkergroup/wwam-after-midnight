...
test("each source has an honest evidence tier and playable source link", () => {
  ...
  const showReads = canon.episodes.map((episode) => `${episode.dossier.summary} ${episode.dossier.whyItMatters || ""}`);
  assert.equal(showReads.some((read) => /for the room breaks|for a room breaks|where an Up In Ya|where a Straight to Steve|\.\s+the chat (?:leaves|stays)/.test(read)), false, "show reads do not retain broken route/chat sentence joins");
...
test("livestream canon surface is wired into the page and route shell", () => {
  ...
  assert.match(ui, /function richerDisplayRead\(e\)/, "machine-shaped summaries receive a varied source-bound guide pass");
  assert.match(ui, /function guideShape\(e\)/, "guide copy changes with the episode format");
  assert.match(ui, /function guideQuote\(z\)/, "guide cards reject decoder fragments that read like broken starts");
  ...
  assert.match(ui, /SOURCE FILES \/\/ .* -> /, "repeat-fan date ranges use a readable ASCII route separator");
  assert.match(ui, /source audio energy\/overlap pass/, "audio-only shelves use plain-language evidence copy");
