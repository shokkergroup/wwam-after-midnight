...
test("the full promoted portfolio reconciles 308 anchors across 69 sources with zero score drift", () => {
  const window = load();
  ...
  assert.equal(chemistry.moments.length, 308);
  assert.equal(engine.metrics.anchorCount, 308);
  ...
  assert.equal(anchors.length, 308);
...
test("reaction evidence is literal-only even when a category sounds like a room reaction", () => {
  ...
  assert.equal(engine.metrics.unknownReactionCount, 295);
  anchors.forEach((anchor) => {
