...
test("current corpus passes canon integrity with deterministic internal-only warnings", () => {
  ...
  assert.equal(first.metrics.characterEvidenceSources, 55);
  assert.equal(first.metrics.ownedEvidenceSources, 129);
  assert.equal(first.metrics.showcaseReceipts, 879);
  assert.equal(first.metrics.loreReceipts, 1091);
  assert.equal(first.metrics.loreGraphNodes, 232);
  assert.equal(first.metrics.loreGraphEdges, 1035);
  assert.equal(first.metrics.clipShorts, 567);
