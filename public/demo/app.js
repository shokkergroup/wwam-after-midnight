(function () {
  ...
  function ensureWatchalongCanonForSource(sourceId) {
    ...
    var whisperReady = window.WWAM_LIVESTREAM_ASR_EXCERPTS ? Promise.resolve() :
      loadDemoScript("wwam-livestream-asr-excerpts.js?v=1.0.5-low-signal-filter");
  ...
  function loadSourceDossier() {
    ...
    sourceDossierLoadPromise = loadArchiveAtlas()
      ...
      .then(function () {
        return loader.load("source-dossier-assets.js?v=1.0.32-late-tail-human-play");
      })
      .then(function () {
  ...
  function fallbackSourceWiki(sourceId, startTime, section) {
    ...
    document.getElementById("modalContent").innerHTML =
      ...
      (moments.length ? (editorialMoments.length ? '<section class="source-dossier-fallback-editorial-cut" aria-labelledby="fallback-editorial-title"><header><div><p class="kicker">FULL-TAPE HUMAN READ</p><h4 id="fallback-editorial-title">THE EDITOR\'S CUT</h4></div><span>' + editorialMoments.length + ' story door' + (editorialMoments.length === 1 ? '' : 's') + '</span></header><div class="source-dossier-fallback-route-grid">' + editorialMoments.map(function (moment, index) { return fallbackRouteButtonMarkup(moment, index); }).join("") + '</div></section>' : '') + (discoveryMoments.length ? '<details class="source-dossier-fallback-discovery"' + (editorialMoments.length ? '' : ' open') + '><summary><span>MORE DOORS // LISTENING LEADS</span><b>' + discoveryMoments.length + ' leads</b></summary><p>These are timestamped listening leads from the full tape. Press play, keep what lands, and use the source player to decide what the moment means.</p><div class="source-dossier-fallback-route-grid">' + discoveryMoments.map(function (moment, index) { return fallbackRouteButtonMarkup(moment, index); }).join("") + '</div></details>' : '') : '<p class="source-dossier-fallback-empty">This source has no safe timestamp receipt in the local bundle yet. The source link remains honest and playable.</p>') + '</section>' +
      '<footer class="source-dossier-fallback-foot"><span>LOCAL SHOW WIKI // SOURCE-LOCKED</span><a href="' + sourceHref + '" target="_blank" rel="noopener">OPEN THE FULL EPISODE ON YOUTUBE &#8599;</a></footer></article>';
    var modal = document.getElementById("tapeModal");
