(function (root) {
  ...
  function mainView(model) {
    ...
    if (model.activeTab === "doctors") return '<section><header class="hu-lane-heading"><p class="hu-eyebrow">THE DOCTORS ARE IN</p><h2>LOOMIS & CHALLIS ON CALL</h2><p>Thirty playable Loomis and Challis bits, each clipped to the exact moment.</p></header><div class="hu-character-columns"><div><h3>DR. LOOMIS // ' + model.loomis.length + '</h3>' + model.loomis.map(function (item) { return receiptCard(item); }).join("") + '</div><div><h3>DR. CHALLIS // ' + model.challis.length + '</h3>' + model.challis.map(function (item) { return receiptCard(item); }).join("") + '</div></div></section>';
